...
// src/utils.ts
function warnAboutExternalBindingsNotFound(bindingsNotFound, typeOfBinding) {
  console.warn(
    `
\x1B[33mWarning:
You have requested ${typeOfBinding} but no local instance of such has been found.
...
The following bindings won't be accessible until then:
 ${[
      ...bindingsNotFound
    ].map((notFound) => ` - ${notFound}`).join("\n")}\x1B[0m

`
  );
}

// src/durableObjects.ts
async function getDOBindingInfo(durableObjects) {
  ...
  const registeredWorkersWithDOs = getRegisteredWorkersWithDOs(
    registeredWorkers,
...
function getRegisteredWorkersWithDOs(registeredWorkers, durableObjectsNames) {
  const registeredWorkersWithDOs = /* @__PURE__ */ new Map();
  for (const workerName of Object.keys(registeredWorkers ?? {})) {
    const worker = registeredWorkers[workerName];
    const containsRequestedDO = !!worker.durableObjects.find(
      ({ name }) => durableObjectsNames.has(name)
    );
...
function collectExternalDurableObjects(registeredWorkersWithDOs, foundDurableObjects) {
  return [...registeredWorkersWithDOs.entries()].flatMap(([workerName, worker]) => {
    const dos = worker.durableObjects;
    return dos.map(({ name, className }) => {
      if (!foundDurableObjects.has(name))
        return;
      return {
        workerName,
...
function generateDurableObjectProxyWorkerScript(externalDOs, registeredWorkersWithDOs) {
  return EXTERNAL_DURABLE_OBJECTS_WORKER_SCRIPT + externalDOs.map(({ workerName, className }) => {
...
function warnAboutLocalDurableObjectsNotFound(durableObjectsNotFound) {
...
function getServiceBindingProxyFetch({
  ...
  return async (request) => {
    ...
    try {
      ...
      return new import_miniflare.Response(respBody, resp);
    } catch {
      return new import_miniflare.Response(
...
function warnAboutServicesNotFound(servicesNotFound) {
...
async function instantiateMiniflare(options) {
  const { workerOptions, durableObjects } = await getDOBindingInfo(options.durableObjects) ?? {};
  const { kvNamespaces, r2Buckets, d1Databases, services, textBindings } = options;
  const bindings = {
...
function monkeyPatchVmModule(bindings) {
  ...
  vmModule.runInContext = (...args) => {
    ...
    if (runtimeContext.process?.env && !runtimeContext.process.env[bindingsProxyHasBeenSetSymbol]) {
      for (const [name, binding] of Object.entries(bindings)) {
        runtimeContext.process.env[name] = binding;
      }
      runtimeContext["Request"] = new Proxy(import_miniflare2.Request, {
        construct(target, args2, newTarget) {
          if (args2.length >= 2 && typeof args2[1] === "object" && args2[1].duplex === void 0) {
            args2[1].duplex = "half";
          }
          return Reflect.construct(target, args2, newTarget);
        }
      });
      runtimeContext["Response"] = import_miniflare2.Response;
      runtimeContext["Headers"] = import_miniflare2.Headers;
