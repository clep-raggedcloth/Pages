...
function isUsingAppRouter(vercelConfig) {
  const isRscRoute = (source) => {
    if (!source)
      return false;
    if (!source.has?.some((h2) => h2.type === "header" && h2.key === "rsc"))
      return false;
    if (source.headers?.["vary"] !== "RSC, Next-Router-State-Tree, Next-Router-Prefetch, Next-Url")
      return false;
    if (!source.continue)
      return false;
    if (!source.override)
  ...
  return noneRoutes.some((route, i3) => {
    const nextRoute = noneRoutes[i3 + 1];
    if (!isRscRoute(route) || !isRscRoute(nextRoute))
      return false;
    if (!route.src.endsWith("/"))
      return false;
    if (!route.dest?.endsWith("/index.rsc"))
      return false;
    if (!nextRoute.src.endsWith("/((?!.+\\.rsc).+?)(?:/)?$"))
      return false;
    if (!nextRoute.dest?.endsWith("/$1.rsc"))
      return false;
    return true;
  });
}
